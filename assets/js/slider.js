function initializeSlider(nav, nav_pagination, button_left, button_right, width_to_change, item_active) {
  let margin_left = 0;

  const prev_slide = (x) => {
    if (margin_left <= 0) {
      margin_left += width_to_change * x;
      nav.style.transform = `translateX(-107.2px)`;
...
let width_to_change_1 = (Number(window.innerWidth) < 1001) ? 490 : 432;
let item_active_1 = 1;

initializeSlider(nav_1, nav_pagination_1, button_left_1, button_right_1, width_to_change_1, item_active_1);
...
let width_to_change_2 = (Number(window.innerWidth) < 1001) ? 490 : 432;
let item_active_2 = 1;

initializeSlider(nav_2, nav_pagination_2, button_left_2, button_right_2, width_to_change_2, item_active_2);
